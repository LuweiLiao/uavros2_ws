...
目前所需 6 个包构建通过、默认 GUI 加载通过；水平悬停、±30°、±60°
以及 ±90° 各自的 10 s 保持通过。但最后回正的完整姿态误差约 5.02°，
未达到 ≤5° 的连续保持门限，**整轮飞行验收仍未通过，尚未推送远端**。
70 s 和 120 s 收敛等待条件下的失败均有保留；没有继续放宽门限。
回正时飞控估计航向与 Gazebo 真值存在约 4.8° 差异，具体根因仍需定位，
未擅自修改指定 ArduPilot 分支的算法。
