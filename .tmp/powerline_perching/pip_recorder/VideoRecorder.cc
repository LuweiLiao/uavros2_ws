...
namespace gz::sim
{
  class VideoRecorderPrivate
  {
    ...
    // Local extension: timestamped sensor pixels, composed before native encoding.
    public: std::mutex imageMutex;
    public: std::deque<std::pair<double, QImage>> images;
    public: bool imageSubscribed{false};
    public: size_t pipFrames{0};
    public: size_t staleFrames{0};
    public: double maxAge{0};
    public: void OnImage(const msgs::Image &_msg)
    {
      if (_msg.pixel_format_type() != msgs::RGB_INT8 ||
          _msg.data().size() < _msg.step() * _msg.height()) return;
      QImage img(reinterpret_cast<const uchar *>(_msg.data().data()),
          _msg.width(), _msg.height(), _msg.step(), QImage::Format_RGB888);
      const double stamp = _msg.header().stamp().sec() +
          _msg.header().stamp().nsec() * 1e-9;
      std::lock_guard<std::mutex> lock(this->imageMutex);
      this->images.emplace_back(stamp, img.copy());
      while (this->images.size() > 60) this->images.pop_front();
    }
    public: void Composite(unsigned int width, unsigned int height)
    {
      const double now = std::chrono::duration<double>(this->simTime).count();
      QImage sensor;
      double age = -1;
      {
        std::lock_guard<std::mutex> lock(this->imageMutex);
        for (auto it = this->images.rbegin(); it != this->images.rend(); ++it)
          if (it->first <= now + 0.002)
          {
            age = now - it->first;
            sensor = it->second;
            break;
      ...
      QImage frame(this->cameraImage.Data<unsigned char>(), width, height,
          width * 3, QImage::Format_RGB888);
      QPainter painter(&frame);
      const int pw = static_cast<int>(width * 0.38);
      const int ph = pw * 3 / 4;
      const int x = width - pw - 18, y = height - ph - 18;
      painter.fillRect(x-3, y-29, pw+6, ph+32, QColor(20, 24, 28));
      painter.setPen(Qt::white);
      QFont font; font.setPixelSize(15); painter.setFont(font);
      const bool valid = !sensor.isNull() && age >= -0.002 && age <= 0.20;
      if (valid)
      {
        painter.drawImage(QRect(x,y,pw,ph), sensor);
        painter.drawText(x+8,y-9,QString("DOWN CAMERA  |  dt %1 ms").arg(qRound(age*1000)));
        ++this->pipFrames;
        this->maxAge = std::max(this->maxAge, age);
      }
      else
      {
        painter.fillRect(x,y,pw,ph,QColor(35,39,43));
        painter.drawText(x+8,y-9,"DOWN CAMERA  |  WAITING / STALE");
        ++this->staleFrames;
      }
    }
  };
...
void VideoRecorderPrivate::Initialize()
{
  ...
  if (this->scene)
    return;
  if (!this->imageSubscribed)
  {
    this->imageSubscribed = this->node.Subscribe(
        "/powerline_perching/vision/annotated_image", &VideoRecorderPrivate::OnImage, this);
    gzmsg << "PIP sensor subscription: " << this->imageSubscribed << std::endl;
  }
...
{
  gzmsg << "PIP statistics: frames=" << this->dataPtr->pipFrames
        << " stale=" << this->dataPtr->staleFrames
        << " max_age=" << this->dataPtr->maxAge << std::endl;
