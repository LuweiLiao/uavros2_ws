...
#include "ArduRotorTiltQuadcopterArm.hh"
...
GZ_REGISTER_MODEL_PLUGIN(ArduRotorTiltQuadcopterArm)
...
public:
    ...
    float arm_servo_speed[MAX_MOTORS];
};

/////////////////////////////////////////////////
ArduRotorTiltQuadcopterArm::ArduRotorTiltQuadcopterArm()
...
/////////////////////////////////////////////////
void ArduRotorTiltQuadcopterArm::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
    GZ_ASSERT(_model, "ArduRotorTiltQuadcopterArm _model pointer is null");
    GZ_ASSERT(_sdf, "ArduRotorTiltQuadcopterArm _sdf pointer is null");
    ...
    if (_sdf->HasElement("arm_servo_num")) {
        this->dataPtr->arm_servo_num = _sdf->Get<int>("arm_servo_num");
        ROS_INFO_STREAM("arm_servo_num:" << this->dataPtr->arm_servo_num);
    }

    if (_sdf->HasElement("arm_servo_pub")) {
        this->dataPtr->arm_servo_pub_name = _sdf->Get<std::string>("arm_servo_pub");
        ROS_INFO_STREAM("arm_servo_pub_name:" << this->dataPtr->arm_servo_pub_name);
    }
    ...
    this->dataPtr->arm_servo_pub = this->rosNode->advertise<mav_msgs::Actuators>("/" + this->dataPtr->arm_servo_pub_name, 10);
    ...
    // iteration.
    this->dataPtr->updateConnection = event::Events::ConnectWorldUpdateBegin(std::bind(&ArduRotorTiltQuadcopterArm::OnUpdate, this));
...
/////////////////////////////////////////////////
void ArduRotorTiltQuadcopterArm::OnUpdate()
{
...
/////////////////////////////////////////////////
void ArduRotorTiltQuadcopterArm::ResetPIDs()
{
...
/////////////////////////////////////////////////
bool ArduRotorTiltQuadcopterArm::InitArduPilotSockets(sdf::ElementPtr _sdf) const
{
...
/////////////////////////////////////////////////
void ArduRotorTiltQuadcopterArm::ApplyMotorForces(const double _dt)
{
    ...
    for (int i = 0; i < this->dataPtr->arm_servo_num; i++) {
        this->dataPtr->arm_servo_speed[i] = (this->dataPtr->arm_servo_speed[i] - 1500) / 1000.0f * 135.0f / 57.3f;
    }
    mav_msgs::Actuators arm_servo_msg;
    arm_servo_msg.angular_velocities.clear();
    for (int i = 0; i < this->dataPtr->arm_servo_num; i++) {
        arm_servo_msg.angular_velocities.push_back(this->dataPtr->arm_servo_speed[i]);
    }
    this->dataPtr->arm_servo_pub.publish(arm_servo_msg);
}

/////////////////////////////////////////////////
void ArduRotorTiltQuadcopterArm::ReceiveMotorCommand()
{
    ...
    if (recvSize == -1) {
        ...
        for (unsigned i = 0; i < this->dataPtr->arm_servo_num; ++i) {
            if (i < MAX_MOTORS) {
                double cmd = ignition::math::clamp(pkt.motorSpeed[i + this->dataPtr->motor_num + this->dataPtr->servo_num], -2.0f, 2.0f);
                this->dataPtr->arm_servo_speed[i] = (cmd - 0.5f) * 1000.0f + 1500.0f;
            } else {
                gzerr << "[" << this->dataPtr->modelName << "] "
                      << "too many motors, skipping [" << i << " > " << MAX_MOTORS << "].\n";
            }
...
/////////////////////////////////////////////////
void ArduRotorTiltQuadcopterArm::SendState() const
{
    // send_fdm
    fdmPacket pkt;
