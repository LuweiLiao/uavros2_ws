...
#include "ArduRotorQuadrupedBicopter.hh"
...
GZ_REGISTER_MODEL_PLUGIN(ArduRotorQuadrupedBicopter)
...
public:
    ...
    std::string    coxa_pub_name;

    ros::Publisher femur_pub;
    std::string    femur_pub_name;

    ros::Publisher tibia_pub;
    std::string    tibia_pub_name;
    ...
    float coxa_speed[MAX_MOTORS];
    float femur_speed[MAX_MOTORS];
    float tibia_speed[MAX_MOTORS];
};

/////////////////////////////////////////////////
ArduRotorQuadrupedBicopter::ArduRotorQuadrupedBicopter()
    : dataPtr(new ArduPilotPluginPrivate)
{
    ...
    for (uint8_t i = 0; i < MAX_MOTORS; i++) {
        this->dataPtr->coxa_speed[i] = 0.5;
    }

    for (uint8_t i = 0; i < MAX_MOTORS; i++) {
        this->dataPtr->femur_speed[i] = 0.5;
    }

    for (uint8_t i = 0; i < MAX_MOTORS; i++) {
        this->dataPtr->tibia_speed[i] = 0.5;
    }
...
/////////////////////////////////////////////////
void ArduRotorQuadrupedBicopter::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
    GZ_ASSERT(_model, "ArduRotorQuadrupedBicopter _model pointer is null");
    GZ_ASSERT(_sdf, "ArduRotorQuadrupedBicopter _sdf pointer is null");
    ...
    if (_sdf->HasElement("coxa_num")) {
        this->dataPtr->coxa_num = _sdf->Get<int>("coxa_num");
        ROS_INFO_STREAM("coxa_num:" << this->dataPtr->coxa_num);
    }

    if (_sdf->HasElement("femur_num")) {
        this->dataPtr->femur_num = _sdf->Get<int>("femur_num");
        ROS_INFO_STREAM("femur_num:" << this->dataPtr->femur_num);
    }

    if (_sdf->HasElement("tibia_num")) {
        this->dataPtr->tibia_num = _sdf->Get<int>("tibia_num");
        ROS_INFO_STREAM("tibia_num:" << this->dataPtr->tibia_num);
    }
    ...
    if (_sdf->HasElement("coxa_pub")) {
        this->dataPtr->coxa_pub_name = _sdf->Get<std::string>("coxa_pub");
        ROS_INFO_STREAM("coxa_pub_name:" << this->dataPtr->coxa_pub_name);
    }

    if (_sdf->HasElement("femur_pub")) {
        this->dataPtr->femur_pub_name = _sdf->Get<std::string>("femur_pub");
        ROS_INFO_STREAM("femur_pub_name:" << this->dataPtr->femur_pub_name);
    }

    if (_sdf->HasElement("tibia_pub")) {
        this->dataPtr->tibia_pub_name = _sdf->Get<std::string>("tibia_pub");
        ROS_INFO_STREAM("tibia_pub_name:" << this->dataPtr->tibia_pub_name);
    }
    ...
    this->dataPtr->coxa_pub  = this->rosNode->advertise<mav_msgs::Actuators>("/" + this->dataPtr->coxa_pub_name, 10);
    this->dataPtr->femur_pub = this->rosNode->advertise<mav_msgs::Actuators>("/" + this->dataPtr->femur_pub_name, 10);
    this->dataPtr->tibia_pub = this->rosNode->advertise<mav_msgs::Actuators>("/" + this->dataPtr->tibia_pub_name, 10);
    ...
    // iteration.
    this->dataPtr->updateConnection = event::Events::ConnectWorldUpdateBegin(std::bind(&ArduRotorQuadrupedBicopter::OnUpdate, this));
...
/////////////////////////////////////////////////
void ArduRotorQuadrupedBicopter::OnUpdate()
{
...
/////////////////////////////////////////////////
void ArduRotorQuadrupedBicopter::ResetPIDs()
{
...
/////////////////////////////////////////////////
bool ArduRotorQuadrupedBicopter::InitArduPilotSockets(sdf::ElementPtr _sdf) const
{
...
/////////////////////////////////////////////////
void ArduRotorQuadrupedBicopter::ApplyMotorForces(const double _dt)
{
    ...
    for (int i = 0; i < this->dataPtr->motor_num; i++) {
        this->dataPtr->motor_speed[i] = this->dataPtr->motor_speed[i];
    }

    for (int i = 0; i < this->dataPtr->servo_num; i++) {
        this->dataPtr->servo_speed[i] = (this->dataPtr->servo_speed[i] - 1500) / 500.0f * 45.0f / 57.3f;
    }

    mav_msgs::Actuators coxa_msg;
    coxa_msg.angular_velocities.clear();
    coxa_msg.angular_velocities.push_back(this->dataPtr->coxa_speed[1 - 1]);
    coxa_msg.angular_velocities.push_back(this->dataPtr->coxa_speed[2 - 1]);
    coxa_msg.angular_velocities.push_back(this->dataPtr->coxa_speed[3 - 1]);
    coxa_msg.angular_velocities.push_back(this->dataPtr->coxa_speed[4 - 1]);
    this->dataPtr->coxa_pub.publish(coxa_msg);

    mav_msgs::Actuators femur_msg;
    femur_msg.angular_velocities.clear();
    femur_msg.angular_velocities.push_back(this->dataPtr->femur_speed[1 - 1]);
    femur_msg.angular_velocities.push_back(this->dataPtr->femur_speed[2 - 1]);
    femur_msg.angular_velocities.push_back(this->dataPtr->femur_speed[3 - 1]);
    femur_msg.angular_velocities.push_back(this->dataPtr->femur_speed[4 - 1]);
    this->dataPtr->femur_pub.publish(femur_msg);

    mav_msgs::Actuators tibia_msg;
    tibia_msg.angular_velocities.clear();
    tibia_msg.angular_velocities.push_back(this->dataPtr->tibia_speed[1 - 1]);
    tibia_msg.angular_velocities.push_back(this->dataPtr->tibia_speed[2 - 1]);
    tibia_msg.angular_velocities.push_back(this->dataPtr->tibia_speed[3 - 1]);
    tibia_msg.angular_velocities.push_back(this->dataPtr->tibia_speed[4 - 1]);
    this->dataPtr->tibia_pub.publish(tibia_msg);

    mav_msgs::Actuators motor_msg;
    motor_msg.angular_velocities.clear();
    motor_msg.angular_velocities.push_back(this->dataPtr->motor_speed[1 - 1]);
    motor_msg.angular_velocities.push_back(this->dataPtr->motor_speed[2 - 1]);
...
/////////////////////////////////////////////////
void ArduRotorQuadrupedBicopter::ReceiveMotorCommand()
{
    ...
    if (recvSize == -1) {
        ...
        for (unsigned i = 0; i < this->dataPtr->motor_num; ++i) {
            if (i < MAX_MOTORS) {
                const double cmd = ignition::math::clamp(pkt.motorSpeed[i + this->dataPtr->coxa_num + this->dataPtr->femur_num + this->dataPtr->tibia_num],
                                                         -1.0f,
                                                         1.0f);
        ...
        for (unsigned i = 0; i < this->dataPtr->servo_num; ++i) {
            if (i < MAX_MOTORS) {
                const double cmd = ignition::math::clamp(pkt.motorSpeed[i + this->dataPtr->coxa_num + this->dataPtr->femur_num + this->dataPtr->tibia_num + this->dataPtr->motor_num],
                                                         -2.0f,
                                                         2.0f);
...
/////////////////////////////////////////////////
void ArduRotorQuadrupedBicopter::SendState() const
{
    // send_fdm
    fdmPacket pkt;
