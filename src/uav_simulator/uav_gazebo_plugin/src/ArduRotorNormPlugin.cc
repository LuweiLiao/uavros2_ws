...
class ArduRotorNormPlugin::Private
{
public:
...
/////////////////////////////////////////////////
ArduRotorNormPlugin::~ArduRotorNormPlugin() = default;

/////////////////////////////////////////////////
void ArduRotorNormPlugin::Configure(
    ...
    gz::sim::EventManager &/*_eventMgr*/)
{
  ...
  {
    gzerr << "[ArduRotorNormPlugin] plugin must be attached to a model\n";
    return;
  }
  ...
  // ROS 1 defaults this transform to identity.  The tsduav_quad SDF supplies
  ...
  gzmsg << "[" << dataPtr->modelName << "] ArduRotorNormPlugin configured; "
        << "waiting for nested IMU entity\n";
}

/////////////////////////////////////////////////
bool ArduRotorNormPlugin::ResolveEntities(
    gz::sim::EntityComponentManager &_ecm)
{
...
/////////////////////////////////////////////////
void ArduRotorNormPlugin::PublishMotorCommand()
{
  if (!dataPtr->motorPublisher || !dataPtr->ardupilotOnline)
    return;

  gz::msgs::Actuators message;
  // Preserve the ROS 1 ArduRotorNormPlugin's four-motor remap exactly:
  // physical RotorS order = [SITL 0, SITL 3, SITL 1, SITL 2].
  if (dataPtr->motorNum == 4)
  {
    message.add_velocity(
        static_cast<double>(std::clamp(dataPtr->motorSpeed[0], -1.0f, 1.0f)) *
        1000.0);
    message.add_velocity(
        static_cast<double>(std::clamp(dataPtr->motorSpeed[3], -1.0f, 1.0f)) *
        1000.0);
    message.add_velocity(
        static_cast<double>(std::clamp(dataPtr->motorSpeed[1], -1.0f, 1.0f)) *
        1000.0);
    message.add_velocity(
        static_cast<double>(std::clamp(dataPtr->motorSpeed[2], -1.0f, 1.0f)) *
        1000.0);
  }
  else
  {
    ...
    {
      message.add_velocity(
          static_cast<double>(std::clamp(dataPtr->motorSpeed[
              static_cast<std::size_t>(i)], -1.0f, 1.0f)) * 1000.0);
    }
  }
...
/////////////////////////////////////////////////
void ArduRotorNormPlugin::PreUpdate(
...
/////////////////////////////////////////////////
void ArduRotorNormPlugin::PostUpdate(
    const gz::sim::UpdateInfo &_info,
...
/////////////////////////////////////////////////
void ArduRotorNormPlugin::SendState(
    const gz::sim::UpdateInfo &_info,
    const gz::sim::EntityComponentManager &_ecm)
{
  gz::msgs::IMU imu;
...
/////////////////////////////////////////////////
void ArduRotorNormPlugin::Reset(
    const gz::sim::UpdateInfo &/*_info*/,
    gz::sim::EntityComponentManager &/*_ecm*/)
{
  dataPtr->connectionTimeoutCount = 0;
...
GZ_ADD_PLUGIN(gazebo::ArduRotorNormPlugin, gz::sim::System,
  gazebo::ArduRotorNormPlugin::ISystemConfigure,
  gazebo::ArduRotorNormPlugin::ISystemPreUpdate,
  gazebo::ArduRotorNormPlugin::ISystemPostUpdate,
  gazebo::ArduRotorNormPlugin::ISystemReset)
GZ_ADD_PLUGIN_ALIAS(gazebo::ArduRotorNormPlugin, "ArduRotorNormPlugin")
