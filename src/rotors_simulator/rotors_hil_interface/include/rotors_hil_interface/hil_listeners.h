...
class HilListeners {
 public:
  ...
  void AirSpeedCallback(
      const geometry_msgs::msg::TwistStamped::ConstSharedPtr& air_speed_msg,
      HilData* hil_data) {
  ...
  /// \param[out] hil_data Pointer to latest data collected for HIL publishing.
  void GroundSpeedCallback(
      const geometry_msgs::msg::TwistStamped::ConstSharedPtr& ground_speed_msg,
      HilData* hil_data) {
    std::lock_guard<std::mutex> lock(mtx_);
  ...
  void PressureCallback(
      const sensor_msgs::msg::FluidPressure::ConstSharedPtr& pressure_msg,
      HilData* hil_data) {
    std::lock_guard<std::mutex> lock(mtx_);
