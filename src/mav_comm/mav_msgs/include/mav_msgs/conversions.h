...
// Keep the ROS 1 public type names inside the original mav_msgs namespace;
// only the generated ROS 2 message namespace is different.
using Actuators = msg::Actuators;
using AttitudeThrust = msg::AttitudeThrust;
using RateThrust = msg::RateThrust;
using RollPitchYawrateThrust = msg::RollPitchYawrateThrust;
using TorqueThrust = msg::TorqueThrust;

inline int64_t timeToNanoseconds(const builtin_interfaces::msg::Time& time) {
  return static_cast<int64_t>(time.sec) * 1000000000LL +
         static_cast<int64_t>(time.nanosec);
}

inline int64_t durationToNanoseconds(
...
inline void timeFromNanoseconds(
    int64_t nanoseconds, builtin_interfaces::msg::Time* time) {
  assert(time != nullptr);
  ...
  time->nanosec = static_cast<uint32_t>(remainder);
}

inline void durationFromNanoseconds(
    int64_t nanoseconds, builtin_interfaces::msg::Duration* duration) {
  ...
  duration->nanosec = static_cast<uint32_t>(remainder);
}
...
inline void eigenOdometryFromMsg(const nav_msgs::msg::Odometry& msg,
  ...
  odometry->timestamp_ns = timeToNanoseconds(msg.header.stamp);
...
inline void eigenTrajectoryPointFromTransformMsg(
  ...
  trajectory_point->timestamp_ns = timeToNanoseconds(msg.header.stamp);
...
inline void eigenTrajectoryPointFromMsg(
  ...
  if (msg.transforms.empty()) {
    std::cerr << "[mav_msgs] MultiDofJointTrajectoryPoint is empty.\n";
    return;
  }

  if (msg.transforms.size() > 1) {
    std::clog << "[mav_msgs] MultiDofJointTrajectoryPoint message should have "
              << "one joint, but has " << msg.transforms.size()
              << ". Using first joint.\n";
  }

  trajectory_point->time_from_start_ns =
      durationToNanoseconds(msg.time_from_start);
...
inline void msgOdometryFromEigen(const EigenOdometry& odometry,
  ...
  if (odometry.timestamp_ns >= 0) {
    timeFromNanoseconds(odometry.timestamp_ns, &msg->header.stamp);
  }
...
inline void msgPoseStampedFromEigenTrajectoryPoint(
  ...
  if (trajectory_point.timestamp_ns >= 0) {
    timeFromNanoseconds(trajectory_point.timestamp_ns, &msg->header.stamp);
  }
...
inline void msgMultiDofJointTrajectoryPointFromEigen(
  ...
  durationFromNanoseconds(trajectory_point.time_from_start_ns,
                          &msg->time_from_start);
...
inline void msgMultiDofJointTrajectoryFromEigen(
  ...
  if (trajectory.empty()) {
    std::cerr << "[mav_msgs] EigenTrajectoryPointVector is empty.\n";
    return;
  }
